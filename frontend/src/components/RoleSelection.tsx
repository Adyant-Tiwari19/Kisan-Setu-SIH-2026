  ...
  return (
    <main className={`auth-page auth-page-${mode}`}>
      ...
      <style>{`
        ...
        .auth-shell { position: relative; display: grid; grid-template-columns: 40% 60%; max-width: 1160px; min-height: 640px; margin: 0 auto; overflow: hidden; border-radius: 2rem; background: #fffdf8; box-shadow: 0 28px 80px rgba(13,27,42,.12); animation: auth-enter .6s ease both; }
        ...
        .auth-form-area { padding: 3.5rem clamp(2rem, 6vw, 5.5rem); background: #fffdf8; }
        ...
        .join-role-card { position: relative; display: flex; min-width: 0; flex-direction: column; overflow: hidden; border: 1px solid #dce3dc; border-radius: 1.35rem; background: #fffdf8; box-shadow: 0 12px 30px rgba(13,27,42,.07); color: var(--ff-navy); cursor: pointer; outline: none; transition: transform .25s ease, border-color .25s ease, box-shadow .25s ease; }
        .join-role-card:hover, .join-role-card:focus-visible { border-color: var(--ff-fresh); box-shadow: 0 18px 38px rgba(27,67,50,.16); transform: translateY(-5px); }
        ...
        .auth-back-home { position: absolute; top: 1rem; left: max(1rem, calc((100% - 1160px) / 2)); z-index: 2; border: 1px solid rgba(27,67,50,.18); border-radius: 999px; background: rgba(255,255,255,.82); padding: .65rem 1rem; color: var(--ff-deep); font: inherit; font-size: .8rem; font-weight: 800; cursor: pointer; }
